...
func GenerateForKube(ctx context.Context, ctrs []*Container, getService, useLongAnnotations, addReservedAnnotations bool) (*v1.Pod, error) {
	// Generate the v1.Pod yaml description
	return simplePodWithV1Containers(ctx, ctrs, getService, useLongAnnotations, addReservedAnnotations)
}

// GenerateForKube takes a slice of libpod containers and generates
// one v1.Pod description
func (p *Pod) GenerateForKube(ctx context.Context, getService, useLongAnnotations, addReservedAnnotations bool) (*v1.Pod, []v1.ServicePort, error) {
	// Generate the v1.Pod yaml description
	var (
	...
	pod, err := p.podWithContainers(ctx, allContainers, ports, hostNetwork, hostUsers, getService, useLongAnnotations, addReservedAnnotations)
	if err != nil {
		return nil, servicePorts, err
	}
...
func (p *Pod) podWithContainers(ctx context.Context, containers []*Container, ports []v1.ContainerPort, hostNetwork, hostUsers, getService, useLongAnnotations, addReservedAnnotations bool) (*v1.Pod, error) {
	...
	for _, ctr := range containers {
		if !ctr.IsInfra() {
			for k, v := range ctr.config.Spec.Annotations {
				if !addReservedAnnotations && (define.IsReservedAnnotation(k) || annotations.IsReservedAnnotation(k)) {
					continue
				}
...
func simplePodWithV1Containers(ctx context.Context, ctrs []*Container, getService, useLongAnnotations, addReservedAnnotations bool) (*v1.Pod, error) {
