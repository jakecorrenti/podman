...
`
	...
	if len(ctrs) >= 1 {
		po, err := libpod.GenerateForKube(ctx, ctrs, options.Service, options.UseLongAnnotations, options.AddReserved)
		if err != nil {
			return nil, err
		}
...
func getKubePods(ctx context.Context, pods []*libpod.Pod, options entities.GenerateKubeOptions) ([][]byte, [][]byte, error) {
	...
	for _, p := range pods {
		po, sp, err := p.GenerateForKube(ctx, options.Service, options.UseLongAnnotations, options.AddReserved)
		if err != nil {
			return nil, nil, err
		}
