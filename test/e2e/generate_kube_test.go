...
USER test1`
	...
	It("podman kube generate --add-reserved on container with --volumes-from", func() {
		ctr1 := "ctr1"
		ctr2 := "ctr2"
		vol1 := filepath.Join(podmanTest.TempDir, "vol-test1")
		...
		kube := podmanTest.Podman([]string{"kube", "generate", "--add-reserved", ctr2})
		kube.WaitWithDefaultTimeout()
		Expect(kube).Should(Exit(0))
	...
	It("podman kube generate --add-reserved on container with --rm", func() {
		ctr := "ctr"
		...
		kube := podmanTest.Podman([]string{"kube", "generate", "--add-reserved", ctr})
		kube.WaitWithDefaultTimeout()
		Expect(kube).Should(Exit(0))
	...
	It("podman kube generate --add-reserved on container with --privileged", func() {
		ctr := "ctr"
	...
	It("podman kube generate --add-reserved on container with --init", func() {
		ctr := "ctr"
	...
	It("podman kube generate --add-reserved on container with --cidfile", func() {
		ctr := "ctr"
	...
	It("podman kube generate --add-reserved on container with --security-opt seccomp=unconfined", func() {
		ctr := "ctr"
	...
	It("podman kube generate --add-reserved on container with --security-opt apparmor=unconfined", func() {
		ctr := "ctr"
	...
	It("podman kube generate --add-reserved on container with --security-opt label=level:s0", func() {
		ctr := "ctr"
	...
	It("podman kube generate --add-reserved on container with --publish-all", func() {
		podmanTest.AddImageToRWStore(ALPINE)
